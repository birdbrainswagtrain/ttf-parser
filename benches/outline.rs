...
fn outline_gvar(bencher: &mut bencher::Bencher) {
    let font_data = std::fs::read("fonts/SourceSansVariable-Roman.ttf").unwrap();
    let font = ttf_parser::Font::from_data(&font_data, 0).unwrap();
    bencher.iter(|| {
        for id in 0..font.number_of_glyphs() {
            let _ = font.outline_glyph(ttf_parser::GlyphId(id), &mut Builder(0));
...
bencher::benchmark_group!(outline_group, outline_glyf, outline_cff, outline_gvar, outline_cff2);
